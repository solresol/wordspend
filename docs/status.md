...
- Added a completed-review import gate for the Butler Book 1 worklist. It checks
  the candidate and source-edition checksums, reconstructs the known blank
  worklist baseline, and rejects changes to immutable candidate material.
- The gate accepts only explicit `accept_proposed_range` or
  `adjust_source_range` decisions with identified reviewers and timezone-aware
  timestamps. Adjusted ranges require notes, and all imported ranges must cover
  the complete source book in order with no gaps or overlaps.
- Successful imports reconstruct Greek text from the checksum-verified source
  edition and remain `paper_facing_eligible=no` with
...
- Extended the existing Butler fine-alignment preparation to Book 3, using the
  published Project Gutenberg translation and checksum-verified Monro and Allen
  source edition already in the corpus.
- Prepared 41 machine-proposed segments covering all 461 Greek Book 3 lines and
  all 4,040 Butler Book 3 tokens. Each row retains source and target text,
  offsets, references, and upstream checksums.
- Prepared a checksum-linked 41-row human-review worklist with every decision,
  reviewed range, reviewer, timestamp, and note field blank. All candidate and
  worklist rows remain paper-facing ineligible and excluded from analysis.
- Verified exact ordered source-line and target-token coverage, byte-identical
  regeneration, blank-worklist rejection without a reviewed output, both
  translation provenance validators, the two-source paper-facing selector,
  and all 30 local unit tests.

## 2026-09-16 progress

- Extended the existing Butler fine-alignment preparation to Book 4, using the
  published Project Gutenberg translation and checksum-verified Monro and Allen
  source edition already in the corpus.
- Prepared 39 machine-proposed segments covering all 544 Greek Book 4 lines and
  all 5,048 Butler Book 4 tokens. Each row retains source and target text,
  offsets, references, and upstream checksums.
- Prepared a checksum-linked 39-row human-review worklist with every decision,
  reviewed range, reviewer, timestamp, and note field blank. All candidate and
  worklist rows remain paper-facing ineligible and excluded from analysis.
...
- Books 1–4 now have 217 candidate and pending-review rows covering 2,493 Greek
